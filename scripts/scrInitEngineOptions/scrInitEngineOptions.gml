...
function scrInitEngineOptions() {
	...
	global.debugVisuals = false; // Enables changing the color/alpha of the player when inf jump/god mode are toggled, make sure to disable this if you want to change the player's image_alpha or image_blend
	...
	global.windowCaptionDef = "I Wanna Super Ability"; // Sets the default window caption
	window_set_caption(global.windowCaptionDef);

	global.startRoom = rWorld1_1; // Sets which room for the game to begin with
...
#macro MD5_STR_ADD "Hello hacker" // Sets what to add to the end of md5 input string to make saves harder to hack, should be set to something unique and hard to predict (similar to setting a password)
