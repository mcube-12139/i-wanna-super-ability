...
function scrSaveGame(argument0) {
	...
	if (savePosition) {
	    ...
	    with (objPlayer) {
	        if (collide_brick(floor(global.savePlayerX),global.savePlayerY)) {
	            global.savePlayerX += 1;
	        }
        
	        if (collide_brick(global.savePlayerX,floor(global.savePlayerY))) {
	            global.savePlayerY += 1;
	        }
        
	        if (collide_brick(floor(global.savePlayerX),floor(global.savePlayerY))) {
	            global.savePlayerX += 1;
	            global.savePlayerY += 1;
	        }
	    }
